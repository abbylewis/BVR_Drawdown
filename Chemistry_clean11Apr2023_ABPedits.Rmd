...
# Iron
fe<-sum_plot3%>%
  mutate(color=ifelse(color=="SFe_mgL","Soluble Fe", "Total Fe"), # Rename the Fe for the legend
         color=factor(color, levels = c("Total Fe", "Soluble Fe")) # Reorder
         )

c <- plot_facet(data = fe, 
                var_name = "Fe_mgL", 
                xint_1 = 19184,
                xint_2 = 19178,
...
# Mn
mn<-sum_plot3%>%
  mutate(color=ifelse(color=="SMn_mgL","Soluble Mn", "Total Mn"), # Rename the Mn for the legend
         color=factor(color, levels = c("Total Mn", "Soluble Mn")) # Reorder
         )

d <- plot_facet(data = mn, 
                var_name = "Mn_mgL", 
                xint_1 = 19184,
                xint_2 = 19170,
...
ph<-sum_plot4%>%
  mutate(color=ifelse(color=="TP","Total P", "Phosphate"), # Rename the Fe for the legend
         color=factor(color, levels = c("Total P", "Phosphate")) # Reorder
         )

aa <- plot_facet(data = ph, 
                var_name = "Phosphorus", 
                xint_1 = 19170,
                xint_2 = 19163,
                color = c("black","black"),
...
# Plots Nitrogen
ni<-sum_plot4%>%
  mutate(color=ifelse(color=="TN","Total N", "Ammonium"), #rename the column for the legend
         color=factor(color, levels = c("Total N", "Ammonium")) # Reorder so totals is a solid line
         )

bb<-plot_facet(data = ni, 
                var_name = "Nitrogen", 
                xint_1 = 19170,
                xint_2 = 19191,
...
# Plots Phytoplankton
pl<-sum_plot4%>%
  mutate(color=ifelse(color=="Total phytoplankton","Total phytoplankton", "Cyanobacteria"), 
         color=factor(color, levels = c("Total phytoplankton", "Cyanobacteria"))
         )

cc<-plot_facet(data = pl, 
                var_name = "Phyto", 
                xint_1 = 19170,
                xint_2 = 19170,
...
cmax<-sum_plot5%>%
  ...
  mutate(color=ifelse(color=="TotalConc_ugL","Total phytoplankton", "Cyanobacteria"), #rename the column for the legend
         color=factor(color, levels = c("Total phytoplankton", "Cyanobacteria")) # Reorder so totals is a solid line
         )

c3 <- plot_facet(data = cmax, 
                var_name = "Cmax_depth", 
                xint_1 = 19163,
                xint_2 = 19178,
                reverse = T,
