...
This file loads in-situ thermistor data and calculates lake physics metrics
...
Load and format thermistor data
```{r}
...
write.csv(schmidt_df, "Schmidt.csv", row.names = F)
```


Calculate maximum buoyancy frequency
```{r}
...
write.csv(buoy_max, "BF.csv", row.names = F)
```


Calculate volume-weighted water temperature
```{r}
...
write.csv(vw_temp, "VW_temp.csv", row.names = F)
```
...
Calculate volume of each layer of the reservoir
```{r}
