...
cat_long <- bvr_pre_05APR21%>%
  ...
  select(Reservoir, Site,DateTime, Reading, Sensor_depth, Rounded_depth_hundreth, Depth_m_13, Reservoir_depth, Distance_above_sediments)
...
for(date in unique(catwalk_format$Date)){
  ...
  baths <- flexible_bathy%>%
    filter(Date==date)
  schmidts <- c(schmidts, 
                schmidt.stability(wtr = temps$Reading, 
